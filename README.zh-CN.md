...
| 你需要什么 | 预构建安装包 | 从源码运行 |
| --- | --- | --- |
| **Node.js 18+** 加入 `PATH` | ✅ 必需 — Clawd 的 agent hooks（Claude Code、CodeBuddy、Kiro、opencode、Pi 等）都会通过 `node` 子进程转发事件。如果没有 Node，hooks 会抛 `ENOENT`，Clawd 回退到日志轮询（精度有限、无权限气泡）。 | ✅ 必需 — 用来安装依赖和运行 Electron。 |
| **Claude Code CLI**（[安装指南](https://docs.claude.com/en/docs/claude-code/quickstart)） | 可选 — 仅当你希望 Clawd 与 Claude Code 集成时需要。其它 agent（Codex CLI、Copilot CLI、Gemini CLI 等）独立工作。 | 可选，与安装包一致。 |
| **git** | 安装包用户不需要。 | 需要，用来 clone 仓库。 |

> **为什么用安装包还需要 Node？** Clawd 应用本身用 Electron 自带的 Node 运行时，但 agent hooks 脚本（例如 `~/.claude/hooks/clawd-state.js`）是被各 CLI 当作**独立 Node 进程**拉起的，需要在调用 agent 的 `PATH` 上能找到 `node`。请从 [nodejs.org](https://nodejs.org/) 下载 Node 18 LTS 或更新版本（或使用 `nvm` / `volta`）后再启动 Clawd。
>
> 如果启动 Clawd 时 `PATH` 里找不到 Node，`设置…` → `通用` 顶部会出现一个黄色横幅，提供一键修复入口。
