...
(function initSettingsI18n(root) {
  const STRINGS = {
    en: {
      ...
      doctorCheckNodeAvailability: "Node.js",
      doctorNodeOk: (nodePath) => `Node.js detected: ${nodePath}`,
      doctorNodeMissing: "Node.js not found — hooks cannot run. Install from nodejs.org",
      doctorNodeMissingHint: "Without Node.js, Clawd cannot communicate with AI agents.",
      doctorNodeInstallTitle: "Quick install (pick one for your OS):",
      doctorNodeInstallDownload: "Or download from nodejs.org →",
      ...
      rowGongfengGenerateWizard: "Setup wizard",
      rowGongfengGenerateWizardDesc: "Generate the 11-hook config wizard HTML and open it in your browser. Required because the plugin's hooks.json is cloud-synced and must be edited via its UI.",
      btnGongfengGenerateWizard: "Generate wizard",
      rowGongfengGenerateUninstall: "Uninstall wizard",
      rowGongfengGenerateUninstallDesc: "List all Clawd:* hooks currently registered with the plugin so you can remove them via the plugin UI.",
      btnGongfengGenerateUninstall: "Generate uninstall list",
      toastGongfengWizardOk: "Wizard generated. Follow the steps in your browser.",
      toastGongfengWizardOpenFailed: "Failed to open wizard: ",
      toastGongfengPluginMissing: "Gongfeng Copilot plugin not detected (~/.gongfeng-copilot/ missing). Install/run it once first.",
      ...
      aboutForkRepoLabel: "Fork Repository",
      aboutUpstreamRepoLabel: "Upstream Repository",
      aboutForkSyncNote: "This fork syncs with the upstream repository for continuous updates.",
    ...
    zh: {
      ...
      doctorCheckTheme: "主题",
      doctorCheckNodeAvailability: "Node.js",
      doctorNodeOk: (nodePath) => `已检测到 Node.js：${nodePath}`,
      doctorNodeMissing: "未检测到 Node.js — hook 无法运行，请从 nodejs.org 安装",
      doctorNodeMissingHint: "没有 Node.js，Clawd 无法与 AI agent 通信。",
      doctorNodeInstallTitle: "快速安装（选择对应系统的命令）：",
      doctorNodeInstallDownload: "或从 nodejs.org 下载安装包 →",
      ...
      rowGongfengGenerateWizard: "配置向导",
      rowGongfengGenerateWizardDesc: "生成 11 个 hook 的配置向导 HTML 并在浏览器中打开。因为插件的 hooks.json 由云端同步，必须通过插件 UI 创建 hook。",
      btnGongfengGenerateWizard: "生成配置向导",
      rowGongfengGenerateUninstall: "卸载向导",
      rowGongfengGenerateUninstallDesc: "列出当前插件中的所有 Clawd:* hook，方便你在插件 UI 中逐条删除。",
      btnGongfengGenerateUninstall: "生成卸载向导",
      toastGongfengWizardOk: "向导已生成，请按照浏览器中的步骤操作。",
      toastGongfengWizardOpenFailed: "打开向导失败: ",
      toastGongfengPluginMissing: "未检测到 Gongfeng Copilot 插件（~/.gongfeng-copilot/ 不存在），请先安装并启动一次插件。",
      rowLanguage: "语言",
      ...
      aboutForkRepoLabel: "Fork 仓库",
      aboutUpstreamRepoLabel: "原始仓库",
      aboutForkSyncNote: "本 Fork 仓库会同步原始仓库功能，持续更新。",
    ...
    "zh-TW": {
      ...
      doctorCheckTheme: "主題",
      doctorCheckNodeAvailability: "Node.js",
      doctorNodeOk: (nodePath) => `已偵測到 Node.js：${nodePath}`,
      doctorNodeMissing: "未偵測到 Node.js — hook 無法執行，請從 nodejs.org 安裝",
      doctorNodeMissingHint: "沒有 Node.js，Clawd 無法與 AI agent 通訊。",
      doctorNodeInstallTitle: "快速安裝（選擇對應系統的指令）：",
      doctorNodeInstallDownload: "或從 nodejs.org 下載安裝包 →",
      ...
      rowGongfengGenerateWizard: "設定精靈",
      rowGongfengGenerateWizardDesc: "產生 11 個 hook 的設定精靈 HTML 並在瀏覽器開啟。插件的 hooks.json 由雲端同步，必須透過插件 UI 建立 hook。",
      btnGongfengGenerateWizard: "產生設定精靈",
      rowGongfengGenerateUninstall: "解除安裝精靈",
      rowGongfengGenerateUninstallDesc: "列出目前插件中所有 Clawd:* hook，方便你在插件 UI 逐條刪除。",
      btnGongfengGenerateUninstall: "產生解除安裝清單",
      toastGongfengWizardOk: "精靈已產生，請依照瀏覽器中的步驟操作。",
      toastGongfengWizardOpenFailed: "開啟精靈失敗: ",
      toastGongfengPluginMissing: "未偵測到 Gongfeng Copilot 插件（~/.gongfeng-copilot/ 不存在），請先安裝並啟動一次插件。",
      rowLanguage: "語言",
      ...
      aboutForkRepoLabel: "Fork 儲存庫",
      aboutUpstreamRepoLabel: "上游儲存庫",
      aboutForkSyncNote: "本 Fork 儲存庫會同步上游儲存庫功能，持續更新。",
    ...
    ko: {
      ...
      doctorCheckTheme: "테마",
      doctorCheckNodeAvailability: "Node.js",
      doctorNodeOk: (nodePath) => `Node.js 감지됨: ${nodePath}`,
      doctorNodeMissing: "Node.js를 찾을 수 없습니다 — hook을 실행할 수 없습니다. nodejs.org에서 설치하세요",
      doctorNodeMissingHint: "Node.js 없이는 Clawd가 AI agent와 통신할 수 없습니다.",
      doctorNodeInstallTitle: "빠른 설치 (OS에 맞는 명령어 선택):",
      doctorNodeInstallDownload: "또는 nodejs.org에서 다운로드 →",
      ...
      rowGongfengGenerateWizard: "설정 마법사",
      rowGongfengGenerateWizardDesc: "11개 hook의 구성 마법사 HTML을 생성하여 브라우저에서 엽니다. 플러그인의 hooks.json은 클라우드 동기화되므로 플러그인 UI를 통해 hook을 생성해야 합니다.",
      btnGongfengGenerateWizard: "마법사 생성",
      rowGongfengGenerateUninstall: "제거 마법사",
      rowGongfengGenerateUninstallDesc: "플러그인에 등록된 모든 Clawd:* hook을 나열하여 플러그인 UI에서 하나씩 삭제할 수 있습니다.",
      btnGongfengGenerateUninstall: "제거 목록 생성",
      toastGongfengWizardOk: "마법사가 생성되었습니다. 브라우저의 단계를 따르세요.",
      toastGongfengWizardOpenFailed: "마법사 열기 실패: ",
      toastGongfengPluginMissing: "Gongfeng Copilot 플러그인이 감지되지 않았습니다(~/.gongfeng-copilot/ 없음). 먼저 설치하고 한 번 실행하세요.",
      rowLanguage: "언어",
      ...
      aboutForkRepoLabel: "Fork 저장소",
      aboutUpstreamRepoLabel: "원본 저장소",
      aboutForkSyncNote: "이 Fork 저장소는 원본 저장소와 동기화되어 지속적으로 업데이트됩니다.",
    ...
    ja: {
      ...
      doctorCheckNodeAvailability: "Node.js",
      doctorNodeOk: (nodePath) => `Node.js 検出済み: ${nodePath}`,
      doctorNodeMissing: "Node.js が見つかりません — hook を実行できません。nodejs.org からインストールしてください",
      doctorNodeMissingHint: "Node.js がないと、Clawd は AI agent と通信できません。",
      doctorNodeInstallTitle: "クイックインストール（OSに合ったコマンドを選択）：",
      doctorNodeInstallDownload: "または nodejs.org からダウンロード →",
      ...
      rowGongfengGenerateWizard: "設定ウィザード",
      rowGongfengGenerateWizardDesc: "11 個の hook 構成ウィザード HTML を生成しブラウザで開きます。プラグインの hooks.json はクラウド同期のため、プラグイン UI 経由で hook を作成する必要があります。",
      btnGongfengGenerateWizard: "ウィザードを生成",
      rowGongfengGenerateUninstall: "アンインストールウィザード",
      rowGongfengGenerateUninstallDesc: "プラグインに登録されている Clawd:* hook を一覧表示し、プラグイン UI から個別に削除できるようにします。",
      btnGongfengGenerateUninstall: "アンインストール一覧を生成",
      toastGongfengWizardOk: "ウィザードを生成しました。ブラウザの手順に従ってください。",
      toastGongfengWizardOpenFailed: "ウィザードを開けませんでした: ",
      toastGongfengPluginMissing: "Gongfeng Copilot プラグインが検出されません (~/.gongfeng-copilot/ が存在しません)。先にインストールして一度起動してください。",
      rowLanguage: "言語",
      ...
      aboutForkRepoLabel: "Fork リポジトリ",
      aboutUpstreamRepoLabel: "上流リポジトリ",
      aboutForkSyncNote: "この Fork は上流リポジトリと同期し、継続的に更新されます。",
