...
> Tag: `v0.7.15` · Release notes: [`docs/releases/release-v0.7.15.md`](docs/releases/release-v0.7.15.md)

### Fixed
- **"Jump to terminal" and state detection failed entirely for the CodeBuddy CN build — its executable is `CodeBuddy CN.exe`, not `codebuddy.exe`.** On Windows the CN (国内版) IDE registers its process as `CodeBuddy CN.exe` (lower-cased to `codebuddy cn.exe` in the process snapshot), but every process-name match point only listed `codebuddy.exe`: `hooks/codebuddy-hook.js`'s `extraTerminals` / `extraEditors` / `agentNames`, and `src/state.js`'s `detectRunningAgentProcesses` name list. As a result the hook's `detectedEditor` never resolved to `"codebuddy"`, so the `editor` field was never sent to the server — meaning the earlier server-side whitelist fix could never take effect and `scheduleTerminalTabFocus` still bailed on `if (!editor) return`. `stablePid` / `agentPid` resolution and restart-time process detection were also broken. Fix: added the `CodeBuddy CN.exe` / `codebuddy cn.exe` / `codebuddy cn` variants to all match points (Windows + macOS + Linux), factored the names into a shared `CODEBUDDY_PROCESS_NAMES` constant, and added regression tests.
- **CodeBuddy hook process could hang on stdin read failure.** `readStdinJson().then(...)` in `codebuddy-hook.js` lacked a `.catch()` handler; if stdin read or JSON parse threw (e.g. pipe closed prematurely), the process would exit with an unhandled rejection or hang indefinitely, blocking the IDE's hook execution pipeline. Added graceful `.catch()` that emits `{}` and exits cleanly so the IDE never stalls.
- **Windows agent-process detection was case-sensitive for CodeBuddy.** The PowerShell script in `detectRunningAgentProcesses` used `-contains` (case-sensitive) against a names list that only included `codebuddy.exe`. If the actual process registered as `CodeBuddy.exe`, detection would fail and the pet could not auto-recover state on restart. Changed to `-icontains` (case-insensitive) and added the `CodeBuddy.exe` variant to the names array.
- **Dead `PermissionRequest` entry in `agents/codebuddy.js` eventMap.** CodeBuddy IDE never emits a `PermissionRequest` event (permission is handled via `PreToolUse.requires_approval`), but the agent config still mapped it to `"notification"`. Removed the dead entry to prevent developer confusion; added explanatory comment.
- **Unix agent-process detection missed `CodeBuddy` (uppercase).** The `pgrep -f 'codebuddy'` pattern in `detectRunningAgentProcesses` is case-sensitive on Linux/macOS. If the binary registers as `CodeBuddy`, the detection would fail. Added `pgrep -x 'CodeBuddy'` fallback to the chain.
- **CodeBuddy Dashboard "Jump to terminal" silently failed — `editor` field dropped by server.** The state route's editor whitelist in `src/server-route-state.js` only accepted `"code"` and `"cursor"`, discarding the `"codebuddy"` value reported by the CodeBuddy hook. This caused `session.editor` to be stored as `null`, making `scheduleTerminalTabFocus` bail out on its first guard (`if (!editor) return`). The VS Code/CodeBuddy terminal-focus extension's `/focus-tab` HTTP request was never sent, so clicking "Jump to terminal" in the Dashboard activated the IDE window but never switched to the correct terminal tab. Fix: added `"codebuddy"` to the server-side editor whitelist.
- **CodeBuddy permission bubble — dual IDE + CLI path.** Investigation against the runtime bundles clarified that `codebuddy` has two distinct approval mechanisms sharing `~/.codebuddy/settings.json`: (1) the **IDE** (腾讯云代码助手扩展) has **no** dedicated `PermissionRequest` event — approval is carried by `PreToolUse`'s `permissionDecision` (`allow`/`deny`/`ask`), and the IDE only sets `tool_input.requires_approval === true` on payloads it wants confirmed; (2) the **`codebuddy` CLI** is a Claude Code fork (`@tencent-ai/codebuddy-code`) that fires a real blocking `PermissionRequest` hook before a tool that needs approval runs (primarily Bash). Fix: `codebuddy-hook.js` intercepts IDE `PreToolUse` payloads with `requires_approval === true` and forwards them to Clawd's blocking `/permission` endpoint (best-effort, and narrowly scoped: bundle inspection of the IDE confirmed `requires_approval` exists **only** on the `execute_command` tool schema — `delete_file` / `write_to_file` / `append_to_file` / `replace_in_file` are `changeFileTools` gated by the IDE's own file-change UI and never carry it, so the IDE bubble can only ever fire for `execute_command`, and even that is usually pre-confirmed by the built-in TerminalExecutor before the hook runs); **`codebuddy-install.js` now also registers a `PermissionRequest` HTTP hook** (`type:"http"` → `/permission`, mirroring the Claude Code integration) that drives the CLI path and is harmless dead config in the IDE. The installer no longer deletes that HTTP hook — it reconciles the URL to the active runtime port — and only scrubs the dead *command* hooks older installs mistakenly wired to `PermissionRequest`. Both paths converge on the same `/permission` endpoint; server unreachable / no-decision / DND / disabled-agent / disabled-bubble all fall back so Clawd never decides on the user's behalf. `src/integration-sync.js` now threads the runtime server port into the CodeBuddy sync.
...
- **"Jump to terminal" still failed for CodeBuddy IDE — precise terminal-tab focus never fired.** Three remaining root causes fixed: (1) `normalizeFocusRequest` editor allow-list expanded with `"codebuddy"`. (2) `installTerminalFocusExtension` now copies extension into `~/.codebuddycn/extensions`. (3) `EXT_VERSION` bumped to `0.1.1` so the fix reaches users who already had `0.1.0` installed.
- **Update check falsely reports "up to date" when a new release exists.** The CI workflow (`build.yml`) created GitHub Releases as `draft: true`, which meant the GitHub API `/releases/latest` endpoint never returned the new version. Changed `draft: true` → `draft: false` so releases are published immediately upon CI completion, making them visible to the update checker.

### Internal / Refactor
- Documented the semantic distinction between `res.destroy()` (dismiss/autoclose — non-blocking IDE fallback) and explicit `"deny"` (app quit — reliable JSON response to prevent hook hang) in `src/permission.js` cleanup path.
- Reworked `codebuddy-install.js` to register the CLI-facing `PermissionRequest` HTTP hook alongside the state command hooks, with `isClawdPermissionUrl` / `isClawdPermissionHook` / `syncPermissionHttpHook` helpers (URL reconciled to the active port) and a narrowed `cleanupStalePermissionRequestCommandHooks` that only scrubs dead *command* hooks. Added dual-path documentation to both `codebuddy-install.js` and `codebuddy-hook.js` headers.
