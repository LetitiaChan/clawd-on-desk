...
### Fixed
- **CodeBuddy IDE permission bubble never appeared — approvals were wired to a non-existent `PermissionRequest` event.** Investigation against the IDE's own logs (`[HookExecutor] Executing 1 unique hooks for event PreToolUse` → `[ToolHookExecutor] PreToolUse hook result: allowed=true, permissionDecision=none` → `[beforeExecute] Permission decision: source=default_allow`) plus the official Hooks docs confirmed CodeBuddy has **no** dedicated `PermissionRequest` event: approval is carried entirely by `PreToolUse`'s `permissionDecision` (`allow`/`deny`/`ask`). The previous implementation registered both a command hook and an HTTP hook on `PermissionRequest` (dead config — never fired) and made `PreToolUse` return the stale `{"decision":"allow"}`, which the IDE parsed as `permissionDecision=none` and auto-allowed. Fix: `codebuddy-hook.js` now intercepts `PreToolUse` payloads whose `tool_input.requires_approval === true`, forwards the request to Clawd's blocking `/permission` endpoint (shared Claude Code branch, `agent_id:"codebuddy"` + process fields), and translates the returned `decision.behavior` into `hookSpecificOutput.permissionDecision` (`allow`/`deny`). Server unreachable / no-decision / DND / disabled-agent / disabled-bubble all fall back to `permissionDecision:"ask"` so the IDE shows its own prompt — Clawd never decides on the user's behalf. `codebuddy-install.js` stops registering the dead `PermissionRequest` hooks and scrubs any left behind by older installs.
