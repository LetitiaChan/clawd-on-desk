...
- **两个回退目标都不可用时，只回收已空的 quick 宿主**：`145e2613` Windows 原生验收记录了「普通宿主隐藏＋来源最小化」时 Esc 仍留下隐藏前台 HWND。Windows 的显式取消／页面失效在归还失败后，先确认 view 归还成功、quick 已隐藏且 `contentView.children` 确实为空，再于普通 fallback 之后重查原生前台仍是该 quick HWND，才销毁这个临时 `BaseWindow`，由系统选择下一个窗口。不销毁／重载共享页面，不恢复最小化窗口，不枚举或强行激活第三方应用；下次快捷键只新建空壳并借入原页面，旧宿主的迟到事件不能影响新宿主。归还失败、残留 child view、hide 失败、原生探针未知／前台已换、已提交跳转、真实 blur、普通打开、app quit 和 macOS 均不走此回收。原生销毁抛错时保留已追踪句柄，不假装销毁成功；系统最终选择及真实收键仍需 Windows 验证。
