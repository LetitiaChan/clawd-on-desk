...
- Windows 连续按快捷键仍作废旧 revision，但**同一次未提交、且原生前台仍属于 quick 宿主的借用**保留最初来源，直到新轮完成 capture。否则第二次 ready 对仍在前台的 quick HWND 调用 `capture(win, null)` 会丢失来源，Esc 错回普通 Dashboard。连续 pending、已 enter 但未 ready 的替换也遵守这条；原生前台换成其他窗口时重新捕获新来源。busy（含 ready 前开始编辑）、普通打开、外部 blur、已提交跳转、就地轮和其他退出不继承旧来源；来源的可见、未最小化、PID 不变校验仍由 restore 执行。这是本地代码/模型测试约束，新补丁仍待 Windows 真机复测。
- **作废一轮不等于把键盘还回去**：Windows 实测 reload 后 quick 宿主已 `visible=false`，却仍是 GetForegroundWindow 与 GUI focus root，普通 Dashboard `document.hasFocus()=false`、来源窗口再也收不到键。因此页面失效（导航 / reload / 加载失败 / 页面消失）与显式取消走同一条归还路径：本轮没有 submitted 时才尝试归还来源（`restore` 自己校验「此刻前台仍是本窗口」）。来源已销毁 / 最小化 / PID 不匹配而归还失败时，退而把键盘交给**当前持有页面且可见**的普通宿主，但这一步必须在 `hide()` **之后**重新验证前台仍然是本 quick host——hide 可能把前台交给系统挑中的另一个窗口，那是用户的去处，不能拿 hide 之前的旧查询当授权。绝不 show/restore 用户没有打开的窗口、不激活无关窗口，页面已销毁时什么都不做。blur、外部点击、真实跳转和用户重新激活普通宿主都不触发归还——那已经是用户自己选的去处。
